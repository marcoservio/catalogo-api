...
public class AuthController(ITokenService tokenService, UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration, ILogger<AuthController> logger) : ControllerBase
{
    ...
    [HttpPost]
    [Route("CreateRole")]
    [Authorize(Policy = nameof(PolicyNames.SuperAdminOnly))]
    public async Task<IActionResult> CreateRole(string roleName)
    {
        ...
        if (!roleExist)
        {
            ...
            else
            {
                ...
                return BadRequest($"Issue adding the new {roleName} role");
            }
        }
    ...
    public async Task<IActionResult> AddUserToRole(string email, string roleName)
    {
        ...
        if(user is not null)
        {
            ...
            if(result.Succeeded)
            {
                ...
                return Ok($"User {user.Email} added to the {roleName} role");
            }
            else
            {
                _logger.LogInformation(1, $"Error: Unable to add user {user.Email} to the {roleName} role");

                return BadRequest($"Error: Unable to add user {user.Email} to the {roleName} role");
            }
        }
    ...
    [Authorize(Policy = nameof(PolicyNames.ExclusiveOnly))]
